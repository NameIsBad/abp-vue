...
import { PermissionsService } from '/@/api/proxy/permission-management';
import {
...
export function usePermissions({ getPropsRef }: UsePermission) {
  const { t } = useI18n();
  const { get, update } = new PermissionsService();
  /** 弹出层标题 */
  const title = ref('');
  /** 权限树 */
  ...
  function handleSavePermission() {
    ...
    return update(props.providerName, props.providerKey, {
