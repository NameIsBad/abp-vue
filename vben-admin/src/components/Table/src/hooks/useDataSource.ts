...
export function useDataSource(
  ...
  async function fetch(opt?: FetchParams) {
    ...
    try {
      ...
      } else if (DEFAULT_BEFORE_FETCH && isFunction(DEFAULT_BEFORE_FETCH)) {
        DEFAULT_BEFORE_FETCH(params);
      }
      let bParams = [] as any[];
      if (beforeParams && isFunction(beforeParams)) {
        bParams = beforeParams() || [];
      }
      const res = await api(...bParams, params);
