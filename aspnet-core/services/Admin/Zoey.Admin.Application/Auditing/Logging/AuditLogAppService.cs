using Zoey.Admin.Auditing.Features;
using Zoey.Admin.Auditing.Permissions;
...
public class AuditLogAppService : AdminAppService, IAuditLogAppService
{
    ...
    public virtual async Task<PagedResultDto<AuditLogDto>> GetListAsync(AuditLogGetByPagedDto input)
    {
        var auditLogCount = await AuditLogRepository
            .GetCountAsync(input.StartTime, input.EndTime,
                ...
                input.ApplicationName, input.ClientIpAddress, input.CorrelationId,
